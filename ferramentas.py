...
def resumo_estatistico(pergunta: str, df: pd.DataFrame) -> str:
    ...
    # Gera o resumo estatístico numérico do DataFrame
    estatisticas_descritivas = df.describe(include='number').transpose().to_string()

    # Template para gerar a resposta com LLM
    template_resposta = PromptTemplate(
        template = """
...
""",
    ...
    # Cadeia de execução com modelo + parser
    cadeia = template_resposta | llm | StrOutputParser()

    # Invoca a cadeia passando os dados
    resposta = cadeia.invoke({
        "pergunta": pergunta,
...
def gerar_grafico(pergunta: str, df: pd.DataFrame) -> str:
    """Utilize esta ferramenta sempre que a pessoa usuária solicitar um gráfico a partir de um DataFrame Pandas (`df`),
    com base em uma instrução do usuário. A instrução pode conter pedidos como: crie um gráfico de média de tempo de entrega por clima,
    ...
    # Gera metadados e amostra dos dados
    colunas_info = "\n".join([f"- {col} ({dtype})" for col, dtype in df.dtypes.items()])
    amostra_dados = df.head(3).to_dict(orient='records')

    # Template para geração do código do gráfico
    template_resposta = PromptTemplate(
        template="""
...
7. Posicione o título à esquerda com `loc='left'`, use `pad=20` e `fontsize=14`;
8. Mantenha os ticks do eixo X sem rotação com `plt.xticks(rotation=0)`;
...
# Funç˜åo para criar ferramentas
...
def criar_ferramentas(df):
    ...
    description="""Utilize esta ferramenta sempre que o usuário solicitar um gráfico a partir de um DataFrame
                ...
                'represente graficamente', entre outros.""",
    return_direct=True)
